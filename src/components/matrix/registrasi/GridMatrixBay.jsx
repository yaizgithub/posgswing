...
import { Button, Card, Drawer, Modal, Skeleton, message, notification } from "antd";
...
import EditFormRegistrasi from "../../registrasi/EditFormRegistrasi";
import PindahBayRegistrasi from "../../registrasi/PindahBayRegistrasi";
...
const GridMatrixBay = (props) => {
    ...
    const [isShowModalRegistrasi, setisShowModalRegistrasi] = useState(false);
    ...
    const [isShowModalEditRegistrasi, setIsShowModalEditRegistrasi] = useState(false);
    const [isModalPindahBay, setisModalPindahBay] = useState(false)

    useEffect(() => { console.log(props.end); }, [props.end, customer, nomorRegistrasi]);
    ...
    const contextMenuClick = async (args) => {
        ...
        if (grid && args.item.id === "edit-registrasi") {
            // grid.copy(true);
        ...
        if (grid && args.item.id === "pindah-bay") {
            // grid.copy(true);
            // console.log(args.rowInfo.rowData);
            // console.log(props.dataParentPO);     
            if (customer.registrasi_id !== undefined) {
                setisModalPindahBay(true);
            } else {
                tampilkanPesan("error", 'No Display Data')
            }
        }
        if (grid && args.item.id === "clear-checkin") {
            // console.log({
            //     a: customer.id,
            //     b: customer.namaBay,
            // });
            await mutateUpdateClearCellMatrixbayData([customer.id, customer.namaBay, 'check-in']);
            // grid.copy(true);
            // console.log(args.rowInfo.rowData);
            // console.log(props.dataParentPO);
            // setisShowModal(true);
        }
        if (grid && args.item.id === "clear-play") {
            // console.log({
            //     a: customer.id,
            //     b: customer.namaBay,
            // });
            await mutateUpdateClearCellMatrixbayData([customer.id, customer.namaBay, 'play']);
            // grid.copy(true);
            // console.log(args.rowInfo.rowData);
            // console.log(props.dataParentPO);
            // setisShowModal(true);
        ...
        if (grid && args.item.id === "clear-booking") {
            ...
            await mutateUpdateClearCellMatrixbayData([customer.id, customer.namaBay, 'booking']);
            // grid.copy(true);
            // console.log(args.rowInfo.rowData);
            // console.log(props.dataParentPO);
            // setisShowModal(true);
    ...
    const onCellSelected = async (args) => {
        ...
        let start = "";
        let end = "";
        await axios.get(baseUrl + `/bay/orderbyid?id=${col}`)
            .then((res) => {
                // console.log();
                start = res.data.data[0].time_start;
                end = res.data.data[0].time_end;
        ...
        dispatch(
            reduxUpdateNumberIdentifikasi({ numberIdentifikasi: infoPlayerArray[2] })
        );

        ///cek total jam
        refetchTotalJamDrivingOrder();

        // ///utk edit data registrasi
        // dispatch(getOneRegistrasi(customer.registrasi_id));
    };
    ...
    const recordDoubleClick = () => {
        ...
        if (customer.status !== undefined && customer.status !== "booking") {
            ...
            if (OneRegistrasiData?.success) {
                ...
                dispatch(
                    reduxUpdateNumberIdentifikasi({
                        numberIdentifikasi: customer.registrasi_id,
                    })
                );
                dispatch(
                    reduxUpdateSelected({
    ...
    const queryCellInfo = (args) => {
        if (args.column.field === "time") {
            ...
            if (args.data.time.match(waktuServer)) {
                ...
                args.cell.style.color = "#062E6F";
                args.cell.style.fontWeight = "500";
                args.cell.style.backgroundColor = "#7CACF8";
            }
        }
    ...
    return (
        <div>
            ...
            >
                <Inject
                    services={[Search, Toolbar, CommandColumn, Page, ContextMenu, Freeze]}
                />
            ...
            <Drawer
                title={
                    <>
                        <Button
                            ...
                            size="small"
                            icon={<CloseOutlined />}
                            onClick={() => setisShowModalRegistrasi(false)}
                        />
                        <span className="pl-3">Registrasi</span>
                    </>
                }
                placement={"right"}
                closable={false}
                onClose={() => setisShowModalRegistrasi(false)}
                open={isShowModalRegistrasi}
                ...
                width={610}
            >
                <Card title="Registrasi Entry" size="small">
                    <AddFormRegistrasi
                        closeModal={() => setisShowModalRegistrasi(false)}
                    />
                    {/* <TabTableRegistrasi /> */}
                </Card>
            </Drawer>
            ...
            {/* MODAL SHOW PINDAH BAY */}
            <Modal
                onCancel={() => setisModalPindahBay(false)}
                open={isModalPindahBay}
                closeIcon={false}
                footer={false}
            >
                <PindahBayRegistrasi
                    closeModal={() => setisModalPindahBay(false)}
                />
            </Modal>
            {/* END MODAL SHOW PINDAH BAY */}
        </div>
    );
};

export default GridMatrixBay;
