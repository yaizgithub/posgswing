...
import { useListRestoOrderReservasiData } from "../../hooks/useListRestoOrderReservasiData";
import "./style.css";
import { useUpdateStatusWhereNullToSatuReservasiRestoData } from "../../hooks/reservasi/useReservasiRestoData";

const { confirm } = Modal;

const TableListReservasiRestoOrder = () => {
    const {userid} = useSelector((state)=>state.auth);

    const [currentTime, setCurrentTime] = useState(new Date());

    ///HOOKs
    const { data } = useListRestoOrderReservasiData(true);
    const {mutateAsync: mutateUpdateStatusWhereNullToSatuReservasiResto} = useUpdateStatusWhereNullToSatuReservasiRestoData();
    ...
    const updateStatusOrderan=async(id)=>{
        ...
        await mutateUpdateStatusWhereNullToSatuReservasiResto([id, data]);
    }
    ...
    return (
        <div>
            <div className="font-semibold">Current Time : {formatTime(currentTime)}</div>
            <Table
...
export default TableListReservasiRestoOrder;
