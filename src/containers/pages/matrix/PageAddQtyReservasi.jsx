...
import { usePostReservasiRestoData } from "../../../hooks/reservasi/useReservasiRestoData";
...
const PageAddQtyReservasi = (props) => {
    ...
    const { mutateAsync: dataPostReservasiRestoData } = usePostReservasiRestoData();
    ...
    const onFinish = async (v) => {
        ...
        await axios
            .get(
                baseUrl +
                `/reservasi-resto/generate?reservasi_id=${matrixSelected.registrasi_id}`
            )
            .then(async (res) => {
                ...
                const currentTime = dayjs(props.tanggal).format("YYYY-MM-DD");
                const waktuPesan = matrixSelected.time;
                const hours = waktuPesan.substring(0, 2);
                const minutes = waktuPesan.substring(3, 5);                    
                // const minutes = //new Date().getMinutes().toString().padStart(2, '0');
                // const seconds = //new Date().getSeconds().toString().padStart(2, '0');
                const waktuAkanDisajikan = `${currentTime} ${hours}:${minutes}`;
                const tgl = dayjs(waktuAkanDisajikan);
                const waktuReservasi = tgl;
                const tenMinutesAgo = tgl.subtract(10, 'minute');
                // console.log(
                //     {
                ...
                let data = {
                    ...
                    status_order: null,
                    remark: `Waktu reservasi di jam ${waktuReservasi.format('HH:mm')} buatkan pesanan di jam ${tenMinutesAgo.format('HH:mm')}`,//v.remark,
                    label_alarm:"10", //maksudnya 10 menit sebelum jam reservasi
                    alarm: tenMinutesAgo.format('YYYY-MM-DD HH:mm'),
                    jam_reservasi: waktuReservasi.format('YYYY-MM-DD HH:mm'),
                    user_id: userid,
                    updator: userid,
                };
                await dataPostReservasiRestoData(data);
...
export default PageAddQtyReservasi;
